...
- **The builder UI** (`ui/builder.html`) — single-file visual workflow editor.
  `ui/embed.py` refreshes its embedded schema/lint-rules/templates.
- **`install.sh`** + **`bin/maestro`** — installer and helper CLI (init/validate/
  status/reset/ui; `maestro <slug>` preflights and points at `/maestro <slug>`).
...
- **Placeholders** are pure text substitution (`${inputs.x}`, `${steps.id.outputs.f}`,
  `${steps.id.branches.k.outputs.f}`, `${config.path}`) — no templating engine. `when:`
  conditions support only `==`, `!=`, truthy, `in [a, b]` (`engine/condctl.py`).
...
- **Editing a skill = editing its `SKILL.md`.** Keep the frontmatter contract and each
  ...
  JSON, short scalars only). Skills resolve their own artifact paths from
  `maestro.config.yaml → artifacts` — callers never pass paths.
...
- **Workflows name skills directly** (`skill: plan`); placeholders are allowed in the
  ...
  Model choice: per-node `model:` → workflow `defaults.model` → `maestro.config.yaml
  models.default`, all through `models.aliases`.
